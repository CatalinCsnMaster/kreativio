...
func testVideoToOneArticleUsingArticle(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadArticle(ctx, tx, false, (*[]*Video)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testVideoToOneSetOpArticleUsingArticle(t *testing.T) {
	var err error
...
var (
	videoDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `ArticleID`: `integer`, `Position`: `integer`, `Label`: `text`, `URL`: `text`}
	_            = bytes.MinRead
)
