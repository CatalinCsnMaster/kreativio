...
func testImageToOneArticleUsingArticle(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadArticle(ctx, tx, false, (*[]*Image)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testImageToOneSetOpArticleUsingArticle(t *testing.T) {
	var err error
...
var (
	imageDBTypes = map[string]string{`ID`: `integer`, `CreatedAt`: `timestamp with time zone`, `UpdatedAt`: `timestamp with time zone`, `ArticleID`: `integer`, `Position`: `integer`, `Label`: `text`, `URL`: `text`}
	_            = bytes.MinRead
)
